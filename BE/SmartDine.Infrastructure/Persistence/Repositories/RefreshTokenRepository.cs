...
public class RefreshTokenRepository : GenericRepository<RefreshToken>, IRefreshTokenRepository
{
    ...
    public async Task<IReadOnlyList<RefreshToken>> GetActiveByUserAsync(int userId, string userType) =>
        await _dbSet.Where(r => r.UserId == userId && r.UserType == userType && !r.IsRevoked && r.ExpiresAt > DateTime.UtcNow)
                    .ToListAsync();

    public async Task RevokeAllByUserAsync(int userId, string userType)
    {
